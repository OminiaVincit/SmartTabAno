...
# File: ComplexityWhiteNoise.R
# Do: get complexity of white noise
...
for (i in 1:target_length) {
  ...
  if (numberDataPoints > samplePoints) {
    ...
    sData1 <- devData$Value[1:samplePoints]
    numberDataPoints <- length(sData1)
    
    minData <- min(sData1)
    maxData <- max(sData1)
    meanData <- mean(sData1)
    std <- sd(sData1)
    
    sData2 <- sData[2:samplePoints]-sData[1:(samplePoints-1)]
    std2 <- sd(sData2)
    ...
    # prin non-linear metric of noise data
    if (minData < maxData) {
      
      # create noise data
      noiseData <- rnorm(numberDataPoints, meanData, std)
            nstd <- std
      ...
      timeLag1 <- getTimeLag(noiseData,numberDataPoints%/%10)
                                      
          ApSaMax <- AppSamEn(noiseData, numberDataPoints, m, timeLag1, nrmax2*nstd)
          ApSa1 <- AppSamEn(noiseData, numberDataPoints, m, timeLag1, rpre1*nstd)
          ApSa2 <- AppSamEn(noiseData, numberDataPoints, m, timeLag1, rpre2*nstd)
          ApSa3 <- AppSamEn(noiseData, numberDataPoints, m, timeLag1, rpre3*nstd)
          ApSa4 <- AppSamEn(noiseData, numberDataPoints, m, timeLag1, rpre4*nstd)
